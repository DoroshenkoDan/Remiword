...
import { ModalFormsBlock } from '../_components/ModalFormsBlock'
import { MeaningsBlock }  from '../_components/MeaningsBlock'
...
export async function generateStaticParams() {
  const topics = getTopicsByCategory('modal-verbs')
  return topics.map((t) => ({ slug: t.slug }))
}

export default async function ModalVerbsTopicPage({ params }: Props) {
  const { slug } = await params
  const topic = getTopicBySlug(slug)

  if (!topic || topic.category !== 'modal-verbs') notFound()

  const t = topic as ModalVerbsTopic
    ...
    <div className="flex flex-col h-[calc(100dvh-56px)] md:h-screen overflow-hidden pt-14 md:pt-0">
      ...
      <ScrollArea className="flex-1 min-h-0 px-4 md:px-6">
        <div className="grid md:grid-cols-[1fr_0.8fr] gap-4 md:gap-6 py-4">
          ...
          <div className="flex flex-col gap-4">
            <GrammarCard>
              <MeaningsBlock meanings={t.meanings} />
            </GrammarCard>
